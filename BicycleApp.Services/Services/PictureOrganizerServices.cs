namespace BicycleApp.Services.Services
{
    ...
    using BicycleApp.Services.Services.Interfaces;
    ...
    public class PictureOrganizerServices : IPictureOrganizerServices
    {
        ...
        /// <summary>
        /// This method receives a file and saves it in a specific folder in the main folder for clients
        /// </summary>
        ...
        /// <param name="clientIdentification">unique identification of the client</param>
        /// <returns>The path to the created file as String</returns>
        public string SaveClientPicture(byte[] file, string fileType, string roothFilePath, string clientIdentification)
        {
            this.ValidateInputData(file, fileType, roothFilePath, clientIdentification);

            string clientsMainFolderName = Path.Combine(roothFilePath, ClientsMainFolderName);

            if (!Directory.Exists(clientsMainFolderName))
            {
                Directory.CreateDirectory(clientsMainFolderName);
            }

            string currentClientMainFolderName = Path.Combine(clientsMainFolderName, clientIdentification);

            if (!Directory.Exists(currentClientMainFolderName))
            {
                Directory.CreateDirectory(currentClientMainFolderName);
            }

            string year = DateTime.Now.Year.ToString();
            string currentYearFolderName = Path.Combine(currentClientMainFolderName, year);
        ...
        /// <summary>
        /// This method receives a file and saves it in a specific folder in the main folder for employees
        /// </summary>
        /// <param name="file">The file that has to be saved</param>
        /// <param name="fileType">The type of the file</param>
        /// <param name="roothFilePath">The filepath to the main directory for files to save</param>
        /// <param name="employeeIdentitycation">unique identification of the employee</param>
        /// <returns>The path to the created file as String</returns>
        public string SaveEmployeePicture(byte[] file, string fileType, string roothFilePath, string employeeIdentitycation)
        {
            this.ValidateInputData(file, fileType, roothFilePath, employeeIdentitycation);

            string employeesMainFolderName = Path.Combine(roothFilePath, EmployeesMainFolderName);

            if (!Directory.Exists(employeesMainFolderName))
            {
                Directory.CreateDirectory(employeesMainFolderName);
            }

            string currentEmployeeMainFolderName = Path.Combine(employeesMainFolderName, employeeIdentitycation);

            if (!Directory.Exists(currentEmployeeMainFolderName))
            {
                Directory.CreateDirectory(currentEmployeeMainFolderName);
            }

            string year = DateTime.Now.Year.ToString();
            string currentYearFolderName = Path.Combine(currentEmployeeMainFolderName, year);
