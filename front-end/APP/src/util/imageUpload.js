...
function fetchImageFile(file, id, role) {
  let apiRole =
    role === "user"
      ? environment.upload_avatar
      : environment.employee_upload_avatar;

  // const API_URL = `https://localhost:7047/api/${apiRole}/add_avatar`;
  const API_URL = environment.BASE_URL + apiRole;
  // console.log(API_URL);
